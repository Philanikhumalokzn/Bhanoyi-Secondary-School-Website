...
const result = await page.evaluate(() => ({
  ...
  hasManagerStep: document.body?.innerText?.includes('Manage House Sporting Squad') || false,
  managerSelectCount: document.querySelectorAll('[data-match-manager-sport]').length,
  managerCardCount: document.querySelectorAll('[data-match-manager-card]').length,
  squadCardCount: document.querySelectorAll('[data-match-squad-card]').length,
  selectedFixtureText: document.querySelector('[data-match-selected-fixture]')?.textContent || '',
  statusText: document.querySelector('[data-match-manager-status]')?.textContent || ''
}));

console.log('FILE_PROBE_RESULT');
