...
1. Copy `.env.example` to `.env` and fill in real values:
   ...
   - `MAIL_FROM` – must use a sender at your verified domain, e.g. `Bhanoyi Secondary School <no-reply@info.bhanoyi.education>`
   - `MAIL_TO` – default inbox for tests (form input overrides it)
2. Install dependencies and start the server:

```powershell
cd tools/resend-email-tester
npm install
npm start
```
...
- This tester sends directly through Resend and does not call the website `/api` routes.